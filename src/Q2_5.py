...
def train_log_grad(X_train: np.ndarray, y_train: np.ndarray, epochs: int, batch_size: int,NN, wandb_run: wandb.Run|None=None,):
    ...
    for e in range(epochs):
        ...
        for L in NN.Layers:
            ...
            dead_trackers.append(np.ones(num_neurons, dtype=bool))
        ...
        for i, (X, y) in enumerate(train_dataloader):
            ...
            for layer_idx, L in enumerate(NN.Layers):
                dead_trackers[layer_idx] &= np.all(L.hk <= 1e-6, axis=0)
        ...
        for tracker in dead_trackers:
            num_dead_neurons += np.sum(tracker)

        wandb.log({'debug/num_dead': num_dead_neurons})
